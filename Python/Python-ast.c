...
stmt_ty
Match(expr_ty test, asdl_seq * orelse, int lineno, int col_offset, PyArena
      *arena)
{
    stmt_ty p;
    if (!test) {
...
ast2obj_stmt(void* _o)
{
    ...
    case Match_kind:
        ...
        if (_PyObject_SetAttrId(result, &PyId_test, value) == -1)
            goto failed;
        Py_DECREF(value);
        value = ast2obj_list(o->v.Match.orelse, ast2obj_stmt);
        if (!value) goto failed;
        if (_PyObject_SetAttrId(result, &PyId_orelse, value) == -1)
            goto failed;
        Py_DECREF(value);
...
obj2ast_stmt(PyObject* obj, stmt_ty* out, PyArena* arena)
{
    ...
    if (isinstance) {
        ...
        if (_PyObject_HasAttrId(obj, &PyId_orelse)) {
            ...
            if (tmp == NULL) goto failed;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Match field \"orelse\" must be a list, not a %.200s", tmp->ob_type->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
            orelse = _Py_asdl_seq_new(len, arena);
            ...
            for (i = 0; i < len; i++) {
                ...
                if (res != 0) goto failed;
                if (len != PyList_GET_SIZE(tmp)) {
                    PyErr_SetString(PyExc_RuntimeError, "Match field \"orelse\" changed size during iteration");
                    goto failed;
                }
                asdl_seq_SET(orelse, i, val);
            }
            ...
            PyErr_SetString(PyExc_TypeError, "required field \"orelse\" missing from Match");
            return 1;
        }
        *out = Match(test, orelse, lineno, col_offset, arena);
        if (*out == NULL) goto failed;
        return 0;
    }
